...
def create_missing_households(apps, schema_editor):
    ...
    missing_users = User.objects.filter(role="household").exclude(pk__in=household_owner_ids)
    Household.objects.bulk_create(Household(owner=user) for user in missing_users.iterator())
