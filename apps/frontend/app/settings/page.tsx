...
export default function SettingsPage() {
  ...
  return <div className="mx-auto max-w-[760px]">
    ...
    <section className="card p-6">
      <div className="mb-6 flex items-center justify-between"><div><h2 className="section-title">Profile</h2><p className="mt-1 text-xs text-slate-500">{user?.email}</p></div><span className={`badge ${user?.email_verified ? "badge-green" : "badge-orange"}`}>{user?.email_verified ? "Email verified" : "Verification pending"}</span></div>
