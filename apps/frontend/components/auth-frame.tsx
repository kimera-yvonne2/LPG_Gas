...
export function AuthFrame({ title, subtitle, children }: { title: string; subtitle: string; children: React.ReactNode }) {
  return <main className="auth-theme relative grid min-h-screen overflow-hidden bg-[#08111f] lg:grid-cols-[.85fr_1.15fr]">
    ...
    <aside className="relative hidden flex-col justify-between overflow-hidden border-r border-white/[.07] bg-[#0a1424] p-12 lg:flex">
      <div><Link href="/" className="inline-flex items-center gap-3"><span className="grid size-11 place-items-center rounded-[15px] bg-gradient-to-br from-orange-300 via-orange-500 to-orange-700 text-white shadow-xl shadow-orange-600/20"><Gauge size={21} /></span><span><strong className="block text-base text-white">Lumora</strong><span className="text-[9px] font-bold uppercase tracking-[.2em] text-slate-500">Gas, made visible</span></span></Link></div>
