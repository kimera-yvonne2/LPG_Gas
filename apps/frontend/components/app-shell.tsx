    ...
    <div className={`app-theme lumora-grid min-h-screen ${lightMode ? "app-theme-light" : ""}`}>
      ...
      <aside className={`app-sidebar fixed inset-y-0 left-0 z-50 flex w-[260px] flex-col border-r border-white/[.07] bg-[#081321]/95 text-white shadow-[20px_0_70px_-50px_rgba(0,0,0,.95)] backdrop-blur-2xl transition-[transform,width] duration-300 lg:translate-x-0 ${collapsed ? "lg:w-20" : "lg:w-[260px]"} ${open ? "translate-x-0" : "-translate-x-full"}`}>
        <div className={`flex h-[64px] items-center border-b border-white/[.07] ${collapsed ? "lg:justify-center lg:px-0" : "px-5"}`}>
          <Link to="/dashboard" className={`flex items-center gap-3 ${collapsed ? "lg:hidden" : ""}`}>
            ...
            <span className={collapsed ? "lg:hidden" : ""}><span className="block font-[Raleway] text-[20px] font-black tracking-[-.035em]">Lumora</span><span className="block text-[10px] font-bold uppercase tracking-[.2em] text-slate-500">Gas, made visible</span></span>
          </Link>
        ...
        <div className={`px-4 pt-6 ${collapsed ? "lg:hidden" : ""}`}><p className="px-3 text-[10px] font-black uppercase tracking-[.22em] text-slate-600">Workspace</p></div>
        <nav className="mt-3 space-y-1 px-3">
          {visibleNavigation.map(({ href, label, icon: Icon }) => {
            const active = pathname === href;
            return <Link key={`${href}-${label}`} to={href} onClick={() => setOpen(false)} title={collapsed ? label : undefined} className={`group flex h-11 items-center gap-3 rounded-[13px] px-3.5 text-[14px] font-bold transition ${collapsed ? "lg:justify-center lg:px-0" : ""} ${active ? "bg-gradient-to-r from-orange-500 to-orange-600 text-white shadow-[0_12px_28px_-16px_rgba(249,115,22,.8)] ring-1 ring-orange-300/20" : "text-slate-400 hover:bg-white/[.045] hover:text-white"}`}><Icon size={20} /><span className={collapsed ? "lg:hidden" : ""}>{label}</span>{active && <ChevronRight size={13} className={`ml-auto ${collapsed ? "lg:hidden" : ""}`} />}</Link>;
          })}
        </nav>
      ...
      <div className={`app-main ml-0 min-h-screen transition-[margin] duration-300 ${collapsed ? "lg:ml-20" : "lg:ml-[260px]"}`}>
        <header className="app-header sticky top-0 z-30 flex h-[64px] items-center gap-4 border-b border-white/[.07] bg-[#081321]/85 px-4 backdrop-blur-2xl sm:px-7">
          <button className="grid size-10 place-items-center rounded-xl border border-white/15 bg-white/[.06] text-white shadow-sm transition hover:bg-white/[.12] lg:hidden" onClick={() => setOpen(true)} aria-label="Open navigation"><Menu size={21} strokeWidth={2.5} /></button>
          <div><div className="text-[14px] font-extrabold text-white">{user.role === "technician" ? "Refill operations" : "Live monitoring"}</div></div>
