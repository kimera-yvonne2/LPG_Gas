...
export type Cylinder = { id: number; household: number; household_name: string; capacity: string; empty_weight: string; current_weight: string; gas_percentage: string; installation_date: string; status: string; created_at: string; updated_at: string };
export type Sensor = { id: number; household: number; cylinder: number | null; esp32_id: string; firmware_version: string; mac_address: string; battery_level: string; online_status: boolean; is_active: boolean; last_seen: string | null };
export type Reading = { id: number; sensor: number; cylinder: number; esp32_id: string; timestamp: string; weight: string; gas_percentage: string; temperature: string; signal_strength: number; gas_leak_detected: boolean };
