...
export type RefillProvider = { id: number; name: string };
export type RefillStatus = "pending" | "accepted" | "in_transit" | "completed" | "cancelled";
export type RefillRequest = { id: number; household: number; cylinder: number; assigned_technician: number | null; customer: { name: string; email: string; phone: string } | null; status: RefillStatus; source: "manual" | "automatic"; requested_at: string; updated_at: string };
