...
export function toTelemetryPoints(readings: Reading[]): TelemetryPoint[] {
  return readings
    .filter(
      (reading) =>
        reading.weight !== null && reading.gas_percentage !== null,
    )
    .map((reading) => {
      ...
      return {
        ...
        gas: Number(reading.gas_percentage),
        weight: Number(reading.weight),
        mq2Raw: Number(reading.mq2_raw),
      };
    })
    .filter(
      (point) =>
        !Number.isNaN(new Date(point.timestamp).getTime()) &&
        Number.isFinite(point.gas) &&
        Number.isFinite(point.weight) &&
        Number.isFinite(point.mq2Raw),
    )
    .sort(
      (a, b) =>
...
export function telemetryErrorMessage(error: unknown): string {
  if (
    ...
    return "You don’t have permission to view telemetry analytics.";
